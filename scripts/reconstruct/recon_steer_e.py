...
def reconstruct_detector_level():
    ...
    ma.reconstructDecay("K*0 =direct=> K+ pi-", cut=f"abs(formula(daughterInvM(0, 1) - invM_Kst)) <= formula(1.5 * fullwidth_Kst)", path=main)

    ma.reconstructDecay("B0 =direct=> K*0 e+:cor e-:cor ?addbrems", cut="[abs(deltaE) <= 0.05] and [Mbc > 5.27]", path=main)
