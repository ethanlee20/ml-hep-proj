...
data_filepaths = [
    data_dir_path.joinpath(path) for path in sys.argv[2:]
]
...
def plot_datafile(data_filepath):
    ...
    datafile_plots_path = generic_plots_dir_path.joinpath(
        data_filepath.stem
    )
    ...
    q_squared_cut_info = zip(
        q_squared_cut_names, q_squared_cut_ranges
    )

    def save_and_close(plot_name, cut_name):
        plt.savefig(
            datafile_plots_path.joinpath(
    ...
    for cut_name, cut_range in q_squared_cut_info:
        cut_data = data[
            (data["q_squared"] > cut_range[0])
            & (data["q_squared"] < cut_range[1])
        ]
