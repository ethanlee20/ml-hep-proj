...
def plot_afb(data, out_dir):
    ...
    plt.errorbar(gen_x, gen_y, yerr=gen_err, fmt='none', ecolor='black', elinewidth=1, capsize=1)
    plt.scatter(gen_x, gen_y, s=4, color="red", alpha=1, label=leg_gen, marker="X")
    plt.errorbar(det_x, det_y, yerr=det_err, fmt='none', ecolor='black', elinewidth=1, capsize=1)
