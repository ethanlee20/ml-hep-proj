...
def plot_afb(data, out_dir, ell):
    if ell == "e":
        d_cos_theta_ell = section(data, only_sig=True, var="costheta_e")
    elif ell == "mu":
        d_cos_theta_ell = section(data, only_sig=True, var="costheta_mu")
    d_q_squared = section(data, only_sig=True, var="q_squared")

    num_points = 150
    gen_x, gen_y, gen_err = calc_afb_of_q_squared(d_cos_theta_ell.loc["gen"], d_q_squared.loc["gen"], num_points)
    det_x, det_y, det_err = calc_afb_of_q_squared(d_cos_theta_ell.loc["det"], d_q_squared.loc["det"], num_points)
    # breakpoint()
    leg_gen = stats_legend(d_cos_theta_ell.loc["gen"], "Generator", show_mean=False, show_rms=False)
    leg_det = stats_legend(d_cos_theta_ell.loc["det"], "Detector", show_mean=False, show_rms=False)
