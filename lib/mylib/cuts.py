...
def kst_inv_mass_cut(df_data, plot_dir):
    def calculate_inv_mass_k_pi():
        df_k_4mom = df_data[
            ["K_m_E", "K_m_px", "K_m_py", "K_m_pz"]
        ]
        df_pi_4mom = df_data[
            ["pi_p_E", "pi_p_px", "pi_p_py", "pi_p_pz"]
        ]
    ...
    def calculate_difference_inv_mass_k_pi_inv_mass_kst():
        ...
        df_data["inv_mass_k_pi - inv_mass_kst"] = (
            df_inv_mass_k_pi - inv_mass_kst
        )
    ...
    def cut_on_inv_mass_k_pi():
        kst_full_width = 0.05
        cut = (
            df_data["inv_mass_k_pi - inv_mass_kst"].abs()
            <= 1.5 * kst_full_width
        )
        return df_data[cut]
